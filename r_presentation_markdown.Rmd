...
You can load packages with the `library` function. _Functions_ execute some series of commands in R. Most of the time we'll use functions that other people wrote, but you can also write custom functions yourself. Functions have one or more _arguments_, also called _parameters_. Let's see this in action with the `library` function:
...
I use the `factor` function to tell R that the `sex` column is a factor. Now type in `df`. Under `sex` R reports `(fctr)`, letting us know that the `sex` column is now a factor.
...
Unlike a lot of the functions above, `cor.test` doesn't accept a `data = ` argument. Instead, we have to give it the columns individually from the dataframe usingthe `$` notation we learned about above:
...
The p-value here is less than 0.05, a traditional cutoff. Thus we conclude that it is unlikely we would see a correlation this strong if there was no relationship between time spent in the central corridot and preference.
